...
def run_test_integers():
    ...
    # Test 2:
    expected = [1, -1, 6, 2, 2, 2, 13, 14, -30, -4]
    answer = integers([(1, -1, 6),
                       (2,'yikes', 'hi', 2),
                       [2, 2.5, 13, 14],
                       'hello',
                       [1.56],
...
def integers(sequence_of_sequences):
    ...
    for k in range(len(sequence_of_sequences)):
        subset = sequence_of_sequences[k]
        for j in range(len(subset)):
            if type(subset[j]) == int:
                list = list + [subset[j]]
    return list
...
def run_test_big_letters():
    ...
    # Test 2:
    expected = 'HLLYETWWYSEEEEFSSZDEHMMCZ'
    answer = big_letters([(3, 1, 4),  # not a string
                          'HeLLo?!',  # HLL
                          (-4, 'Yikes', 10),  # not a string
                          [],  # not a string
                          [2,1,'oof'],  # not a string
                          'YEeT',  #YET
                          ['OOPS'],  # not a string
                          '1 WoW !',  # WW
                          'Yng',  # Y
